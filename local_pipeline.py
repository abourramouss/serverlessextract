...
class LithopsExecutor(Executor):
    ...
    def execute_map(self, runner: Callable, parameters) -> None:
        futures = self._executor.map(runner, parameters)
        self._executor.get_result(fs=futures)
...
def remove_cached(parameters):
    ...
    if os.path.exists(os.path.dirname(parameters[ImagingStep.__name__]["output_dir"])):
        images = os.listdir(
...
class LocalDataSource(DataSource):
    def download_file(
        ...
        if read_path == write_path:
            print("Same path, skipping download")
        else:
...
class Pipeline:
    ...
    def prepare_parameters(self, parameters):
        ...
        # Download the lua and source db files
        self._datasource.download_file(
            parameters[RebinningStep.__name__]["lua_file_dir"]["origin"],
            parameters[RebinningStep.__name__]["lua_file_dir"]["destination"],
        )
        self._datasource.download_directory(
            parameters[CalibrationStep.__name__]["sourcedb_directory"]["origin"],
            parameters[CalibrationStep.__name__]["sourcedb_directory"]["destination"],
        )
    ...
    @classmethod
    def create_and_run(cls, parameters, executor_type, datasource_type):
        if executor_type == "local":
            executor = LocalExecutor()
        elif executor_type == "lithops":
        ...
        elif datasource_type == "lithops":
            datasource = LithopsDataSource()
        else:
            raise ValueError(f"Unknown datasource type: {datasource_type}")

        pipeline = cls(parameters, executor, datasource)
        pipeline.run()

    def run(self):
        self._executor.execute(Pipeline.run_pipeline, self.parameters)
        # self.run_imaging()
    ...
    parameters = {
        "parameters": {
            "RebinningStep": {
                ...
                "lua_file_dir": {
                    "origin": PurePosixPath(
                        "/home/ayman/Downloads/pipeline/parameters/rebinning/STEP1-NenuFAR64C1S.lua"
                    ),
            ...
            "CalibrationStep": {
                ...
                "sourcedb_directory": {
                    "origin": sourcedb_directory,
                    "destination": sourcedb_directory,
                },
            },
            "SubstractionStep": {
